...
Proof.
  intros. unfold transfer'_balance_sender1_def. intros.

  transfer'_start_new_exec. simplify_all.
...
Proof.
  intros. unfold transfer'_balance_sender2_def. intros.
...
Proof.
  intros. unfold transfer'_balance_recipient_def. intros.
...
Definition transfer'_is_error_full_def (recipient : address) (amount : uint256) (ll: LedgerLRecord rec): Prop.
  err (transfer' rec def recipient amount) ll | "balanceOf" "msg_sender".
  (* balanceOf[[msg->sender]] >= amount *)
  set (req1  := xIntGeb (balanceOf[msg_sender]) amount).
  con (negb (req1)).
Defined.

Lemma transfer'_is_full_error  (recipient : address) (amount : uint256) (ll: LedgerLRecord rec): 
      transfer'_is_error_full_def recipient amount ll.
Proof.
  intros. unfold transfer'_is_error_full_def. intros.

  transfer'_start_new_eval. simplify_all.
  
  destruct_ledger ll. subst P. rewrite ?toValue_false. rewrite ?toValue_uni_false.
  
  time (solve_full_error; progress try_auto).
