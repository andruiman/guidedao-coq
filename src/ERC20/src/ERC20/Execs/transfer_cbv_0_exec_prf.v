...
Require Import ERC20.Execs.transfer_cbv_0.
...
(* Elpi FullPrint transfer_cbv_0_exec(* _sig_beta *). *)

Lemma transfer_cbv_0_exec_prf: forall (recipient : address)(amount : uint256) (l : LedgerLRecord rec),
  transfer_cbv_0_exec  recipient amount l = 
  exec_state (Uinterpreter (transfer_cbv_0 rec def  _ _   recipient amount)) l .
Proof with 
           match goal with
            | |- ?x = ?y => let t := type of x in exact_no_check (eq_refl (A:=t) x)
           end.
  intros.
  replace (exec_state (Uinterpreter (transfer_cbv_0 rec def _ _   recipient amount)) l ) with 
    (proj1_sig (transfer_cbv_0_exec_sig_beta  recipient amount l )) 
                 by apply (proj2_sig (transfer_cbv_0_exec_sig_beta  recipient amount l ))...
...
#[global] Instance _ex_transfer_cbv_0: ExecsIndex _ _ _ (@transfer_cbv_0) :=
{|
  __exec := @transfer_cbv_0_exec;
  __exec_prf := @transfer_cbv_0_exec_prf
|}.

(* *)
Lemma transfer_cbv_exec_prf: forall (recipient : address)(amount : uint256) (l : LedgerLRecord rec),
  transfer_cbv_0_exec  recipient amount l = 
...
Proof.
  ...
  apply   transfer_cbv_0_exec_prf.
Qed.

#[global] Instance _ex_transfer_cbv: ExecsIndex _ _ _ (@transfer) :=
{|
  __exec := @transfer_cbv_0_exec;
  __exec_prf := @transfer_cbv_exec_prf
|}.

 (* *)

 Elpi TacticsGenerate Execs 0 ERC20 ERC20 Execs Evals transfer " recipient amount" "Tactics" "/Users/andruiman/devel/guidedao-coq//src/ERC20/src/ERC20/Tactics/" "Functions" "Functions" "ERC20." .
